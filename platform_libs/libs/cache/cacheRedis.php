...
class cacheRedis{
	 ...
	 */
	public function set($key, $value, $timeOut = 0,$type='json',$compression = false) {
		if($type=='serialize')
		{
		...
		//压缩
		$compression && $value = gzdeflate($value);
	 ...
	 * @param string $key KEY名称
	 */
	public function get($key,$type='json',$compression = false) {
		$result = $this->redis->get($key);
		//解压
		$compression && $result = gzinflate($result);
		if($type=='serialize')
		{
